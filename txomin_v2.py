...
# Clave AEMET: configura en .streamlit/secrets.toml → AEMET_API_KEY = "tu_clave"
# Solicítala gratis en: https://opendata.aemet.es/centrodedescargas/inicio
try:
...
.msub {
    ...
    color: #991B1B; display: block; margin-top: 3px;
}
.msub2 {
    font-size: 0.72rem; font-weight: 600;
    color: #64748B; display: block; margin-top: 2px;
}
...
.sem-luz-verde::after   { background: #10B981; box-shadow: 0 0 0 0 #10B981; }
...
def deg_to_compass(deg):
    """Convierte grados a rosa náutica en español."""
...
def dir_arrow(deg):
    """Flecha Unicode para dirección."""
    if deg is None or (isinstance(deg, float) and math.isnan(deg)):
        return ""
    arrows = ["↓","↙","←","↖","↑","↗","→","↘"]
    return arrows[round(float(deg) / 45) % 8]

def safe(val, dec=1, unit="", default="—"):
...
def tide_info(dt):
    """
    Estimación de marea para Mutriku (Golfo de Bizkaia).
    Modelo M2 calibrado para el puerto de Bilbao (referencia más cercana).
    - Amplitud M2: 1.76 m (rango medio ~3.5 m)
    - Periodo M2: 12 h 25 min 14 s (44714 s)
    - Fase empírica calibrada para la costa de Gipuzkoa
    NOTA: aproximación ±30 min. Para datos exactos consulta Puertos del Estado.
    """
    t = dt.timestamp()
    AMP     = 1.76
    PERIOD  = 44714.0   # segundos
    PHASE   = 5.4       # radianes — calibrado empíricamente para Bilbao/Gipuzkoa

    h      = AMP * math.cos(2 * math.pi * t / PERIOD - PHASE)
    h_next = AMP * math.cos(2 * math.pi * (t + 1800) / PERIOD - PHASE)
    rising = h_next > h

    # Altura en metros (referencia 0 = bajamar astronómica)
    height = round(h + AMP + 0.3, 2)   # → rango aprox. 0.3–4.1 m

    if h > AMP * 0.80:
        label, emoji = "PLEAMAR",  "🌊"
    elif h < -AMP * 0.80:
        label, emoji = "BAJAMAR",  "🏖️"
    elif rising:
        label, emoji = "SUBIENDO", "↗️"
    else:
        label, emoji = "BAJANDO",  "↘️"

    return height, label, emoji, rising

def fish_score(wind_kmh, wave_m, rising, temp, pressure):
    """
    Puntuación heurística de actividad pesquera (0–10).
    Basada en condicionantes conocidos para el Cantábrico.
    """
    s = 0
    def ok(v): return v is not None and not (isinstance(v, float) and math.isnan(v))

    # Viento: calma = mejor mordida
    if ok(wind_kmh):
        if wind_kmh < 12:  s += 2
        elif wind_kmh < 22: s += 1

    # Oleaje: mar llana favorece pesca costera
    if ok(wave_m):
        if wave_m < 0.7:   s += 2
        elif wave_m < 1.3: s += 1

    # Marea: entrante es la mejor para el Cantábrico
    if rising: s += 2

    # Temperatura: óptimo 14–20 °C para espáridos y besugos
    if ok(temp):
        if 14 <= temp <= 20: s += 2
        elif 11 <= temp <= 23: s += 1

    # Presión: alta y estable es buen signo
    if ok(pressure):
...
def score_ui(s):
    """Devuelve (estrellas, etiqueta, color) según puntuación."""
...
def semaforo(v_racha, ola, presion, alerts):
    """
    Lógica de semáforo:
      ROJO    → alertas AEMET Severe/Extreme  | racha >55 km/h | ola >2.5 m
      AMARILLO→ alertas Moderate | racha >35 km/h | ola >1.5 m | presión <995
      VERDE   → todo lo demás
    """
    nivel = "verde"
    razones = []
    ...
    # Alertas AEMET oficiales (prioridad máxima)
    for a in alerts:
        sev = a.get("severity", "")
    ...
    if ok(v_racha):
        if v_racha > 55 and nivel != "rojo":
            nivel = "rojo"
            razones.append(f"Racha extrema: {v_racha:.0f} km/h")
        elif v_racha > 35 and nivel == "verde":
            nivel = "amarillo"
            razones.append(f"Racha fuerte: {v_racha:.0f} km/h")

    if ok(ola):
        if ola > 2.5 and nivel != "rojo":
            nivel = "rojo"
            razones.append(f"Oleaje peligroso: {ola:.1f} m")
        elif ola > 1.5 and nivel == "verde":
            nivel = "amarillo"
            razones.append(f"Oleaje elevado: {ola:.1f} m")

    if ok(presion) and presion < 995 and nivel == "verde":
        nivel = "amarillo"
        razones.append(f"Presión muy baja: {presion:.0f} hPa")
...
# ══════════════════════════════════════════════════════════════════════
#  CARGA DE DATOS — con gestión de errores explícita
# ══════════════════════════════════════════════════════════════════════
@st.cache_data(ttl=600)
def fetch_meteo():
    """
    Fuente principal: Open-Meteo (viento + oleaje + corriente + temperatura).
    TTL 10 minutos. Errores reportados, nunca silenciados.
    """
    errors = []
    ...
    try:
        rw = requests.get(url_w, timeout=9)
        rw.raise_for_status()
        dw = rw.json()
    except requests.exceptions.Timeout:
        errors.append("⏱️ Timeout al obtener datos de viento (Open-Meteo).")
        return pd.DataFrame(), errors
    except requests.exceptions.HTTPError as e:
        errors.append(f"⛔ Error HTTP en datos de viento: {e}")
        return pd.DataFrame(), errors
    except Exception as e:
        errors.append(f"⛔ Error inesperado (viento): {e}")
        return pd.DataFrame(), errors

    try:
        rm = requests.get(url_m, timeout=9)
        rm.raise_for_status()
        dm = rm.json()
    except requests.exceptions.Timeout:
        errors.append("⏱️ Timeout al obtener datos marinos (Open-Meteo Marine).")
        return pd.DataFrame(), errors
    except requests.exceptions.HTTPError as e:
        errors.append(f"⛔ Error HTTP en datos marinos: {e}")
        return pd.DataFrame(), errors
    except Exception as e:
        errors.append(f"⛔ Error inesperado (marino): {e}")
        return pd.DataFrame(), errors
    ...
    try:
        ...
        # Localizar en timezone de Madrid (manejo de cambios de hora DST)
        try:
...
def fetch_aemet(api_key):
    """
    Alertas reales de AEMET para Euskadi (área 61) en formato CAP.
    TTL 1 hora. Sin API key → degrada gracefulmente.
    """
    if not api_key:
        return [], None
    ...
    try:
        ...
        if meta.get("estado") != 200:
            return [], f"AEMET respondió: {meta.get('descripcion', 'error desconocido')}"

        # Segunda petición: datos CAP reales
        r2 = requests.get(meta["datos"], headers=headers, timeout=12)
        r2.raise_for_status()

        # Parsear XML CAP (estándar OASIS)
        ...
        return [], f"AEMET no disponible: {str(e)}"
...
# Fila más cercana a la hora actual
df['_diff'] = (df['time'] - ahora).abs()
idx = df['_diff'].idxmin()
now = df.loc[idx]
df.drop(columns=['_diff'], inplace=True)

# Valores actuales como float (con protección NaN)
def fv(series_val):
    try:
        v = float(series_val)
...
ola      = fv(now['ola'])
ola_per  = fv(now['ola_per'])
ola_dir  = fv(now['ola_dir'])
corr_vel = fv(now['corr_vel'])   # m/s → km/h al mostrar
...
<div class='metric-grid'>
    ...
    <div class='mbox'>
        ...
        <span class='msub2'>Estimación M2 (±30 min)</span>
    </div>
...
# ══════════════════════════════════════════════════════════════════════
#  SEMÁFORO DE SEGURIDAD MARÍTIMA
# ══════════════════════════════════════════════════════════════════════
...
SEM = {
    "verde": {
        ...
        "luz":    "sem-luz-verde",
        "titulo": "✅ RECOMENDABLE",
        "sub":    "CONDICIONES FAVORABLES",
    },
    "amarillo": {
        "box":    "sem-amarillo",
        "luz":    "sem-luz-amarillo",
        "titulo": "⚠️ PRECAUCIÓN",
        "sub":    "SALIR CON PRECAUCIÓN Y EQUIPO COMPLETO",
    },
    "rojo": {
        "box":    "sem-rojo",
        "luz":    "sem-luz-rojo",
        "titulo": "🚫 NO RECOMENDABLE",
        "sub":    "CONDICIONES ADVERSAS — NO SALIR",
    },
}
s = SEM[nivel]
...
if not AEMET_KEY:
    ...
    <div class='alerta alerta-info' style='margin-top:14px;'>
        ...
        Para activarlas, solicita tu clave gratis en
...
<div class='pie'>
    ...
    Próxima actualización automática en ~10 min
</div>
""", unsafe_allow_html=True)
