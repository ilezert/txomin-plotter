...
# --- 1. CONFIGURACIÓN ---
st.set_page_config(page_title="Txomin v.31.2 - Mutriku Pro", page_icon="🔱", layout="wide")
...
# CSS sin espacios traicioneros
st.markdown(f"""
<style>
.stApp {{background-image: url("{IMG_FONDO_MAR}"); background-size: cover; background-attachment: fixed; background-position: center;}}
...
.metric-card {{background: rgba(255, 255, 255, 0.15); backdrop-filter: blur(5px); border-radius: 15px; padding: 15px; text-align: center; color: white !important; border: 1px solid rgba(255, 255, 255, 0.3);}}
.metric-card h3 {{color: white; margin-bottom: 5px; font-size: 1.1rem;}}
.metric-card h2 {{color: #FBBF24; font-size: 2.2rem; margin: 0;}}
...
.hour-card {{flex: 0 0 auto; width: 170px; background: rgba(255, 255, 255, 0.2); backdrop-filter: blur(8px); border: 1px solid rgba(255,255,255,0.4); border-radius: 15px; padding: 15px; text-align: center; color: white; scroll-snap-align: start; box-shadow: 2px 2px 10px rgba(0,0,0,0.2);}}
...
.rec-image {{border-radius: 10px; width: 100%; height: 150px; object-fit: cover; margin-bottom: 15px; border: 2px solid #BAE6FD;}}
...
def recomendacion_especie_real(ola, viento):
    if ola > 0.8 and ola < 1.8: return "🐟 SARGO", "Consejo: Busca puestas con espuma blanca constante. Usa macizo de anchoa.", IMG_SARGO
    if ola < 0.7 and viento < 12: return "🦑 CHIPIRÓN", "Consejo: Mareas muertas y mar plato. Poteras de tamaño 2.0 en calma.", IMG_CHIPIRON
    if viento >= 10 and ola < 1.3: return "🐠 CHICHARRO", "Consejo: El viento riza la superficie; ideal para plumillas blancas a 3 nudos.", IMG_CHICHARRO
    return "🦂 CABRARROCA", "Consejo: Fondea en piedra dura pura. Chambel con plomo pesado y tiras de chipirón.", IMG_CABRARROCA
...
# --- 3. INTERFAZ ---
st.title("🔱 Txomin v.31.2 - Mutriku Pro")
...
with tab0:
    if dm_m and 'hourly' in dm_m and dw_m and 'list' in dw_m:
        ...
        st.markdown(f"<div class='main-card'><h1 style='margin:0; font-size: 2.8rem;'>MUTRIKU AHORA</h1><p style='font-size:1.4rem; opacity:0.9;'>Previsión Táctica: {datetime.now().strftime('%H:%M')}</p></div>", unsafe_allow_html=True)

        c1, c2, c3, c4 = st.columns(4)
        with c1: st.markdown(f"<div class='metric-card'><h3>🌬️ VIENTO</h3><h2>{v_viento:.1f} km/h</h2><p>{d_viento}</p></div>", unsafe_allow_html=True)
        with c2: st.markdown(f"<div class='metric-card'><h3>🌊 OLA</h3><h2>{ola_act:.1f} m</h2><p>Dir: {ola_dir}°</p></div>", unsafe_allow_html=True)
        with c3: st.markdown(f"<div class='metric-card'><h3>🌡️ AGUA</h3><h2>{temp_agua:.1f} °C</h2><p>Superficie</p></div>", unsafe_allow_html=True)
        with c4: st.markdown(f"<div class='metric-card'><h3>💧 CORRIENTE</h3><h2>{v_corr:.1f} km/h</h2><p>Velocidad</p></div>", unsafe_allow_html=True)
        ...
        with col_eval:
            st.markdown(f"<div class='eval-container'><h3 style='margin:0 0 10px 0; color:#0369A1;'>📈 ACTIVIDAD DE PESCA</h3><h2 style='margin:0;'>{eval_txt}</h2><p style='margin-bottom:0;'>{eval_desc}</p></div>", unsafe_allow_html=True)
            
        with col_esp:
            st.markdown(f"<div class='eval-container'><img src='{esp_img}' class='rec-image'/><h2 style='margin:0; color:#059669;'>{esp_nombre}</h2><p><b>Consejo Táctico:</b> {esp_tip}</p></div>", unsafe_allow_html=True)
        ...
        for i in range(0, min(8, len(dw_m['list']))):
            ...
            if idx_marine < len(dm_m['hourly']['wave_height']):
                ...
                # HTML en una sola línea plana para evitar que Markdown lo rompa
                html_carrusel += f"<div class='hour-card'><h4>{hora_dt.strftime('%H:%M')}</h4><p>🌬️ {c_viento:.1f} km/h<br>{c_dir_v}</p><p>🌊 {c_ola:.1f} m</p><div class='rec-badge'>{c_esp.split(' ')[1]}</div></div>"
